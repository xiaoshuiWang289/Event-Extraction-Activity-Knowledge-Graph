...
#     print(raw_dir)../../data/final/raw_data
    ...
    for _now_id, _candidate_id in kf.split(stack_examples):#raw_stack.json
    ...
    for _ex in preliminary_examples:#preliminary_pred_triggers_pred_roles.json
        tmp_sent = _ex['sentence']
        candidate_triggers = []
    ...
    for _ex in test_examples:#sentences.json
        tmp_sent = _ex['sentence']
        candidate_triggers = []
        for _t in triggers.keys():
            if _t in tmp_sent:
    ...
    print(len(train), len(dev))#1964 492
